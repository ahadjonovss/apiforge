...
export function ImportDialog({ workspaceId, teams, open, onClose, onImported }: Props) {
  ...
  const read = async (file: File) => {
    ...
    try {
      ...
      setPlan(null)
      setError(
        failure instanceof ImportFailure ? failure.message : "Faylni o'qib bo'lmadi",
      )
    }
  }
  ...
  const apply = async () => {
    ...
    try {
      ...
      setPending(false)
      setError(failure instanceof Error ? failure.message : 'Import bajarilmadi')
    }
  }
  ...
  return (
    <Modal
      open={open}
      title="Postman'dan import"
      description="Collection v2.1 JSON faylini tanlang"
      onClose={() => {
        reset()
        onClose()
      }}
    >
      <div className="flex max-h-[70vh] flex-col gap-3 overflow-auto">
        ...
        <Button variant="outline" size="lg" block onClick={() => fileInput.current?.click()}>
          ...
          {plan ? 'Boshqa fayl tanlash' : 'Fayl tanlash'}
        </Button>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-[11px] text-muted-foreground">
                    {plan.folders.length} papka · {plan.endpoints.length} endpoint ·{' '}
                    {plan.collection.variables.length} o'zgaruvchi
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="rounded-md border border-status-redirect/40 bg-status-redirect/10 p-3">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[11px] font-medium text-status-redirect">
                    Ko'chirilmagan yoki o'zgartirilgan joylar
                  </p>
                </div>
